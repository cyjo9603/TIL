...
function solution(lines) {
    const times = lines
        ...
        const completeTime = Number(splitLine[1].substr(-6, 6)) * 1000 + Number(splitLine[1].substr(3, 2)) * 60000 + Number(splitLine[1].substr(0, 2)) * 3600000;
        const startTime = completeTime - Number(splitLine[2].substr(0, splitLine[2].length - 1)) * 1000 + 1;
        return [startTime, completeTime];
    })
        .sort((a, b) => a[0] - b[0]);
    const maxCount = times.reduce((count, time) => {
        let currentCount = 0;
        for (let i = 0; i < times.length; i++) {
            if ((time[0] - 1000 > times[i][0] && time[0] - 1000 < times[i][1]) ||
